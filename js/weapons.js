...
class Weapon {
    ...
    getCooldown() {
        return (
            this.baseCooldown *
            (this.player ? this.player.cooldownModifier || 1.0 : 1.0)
        );
    }
    ...
    getDamage() {
        ...
        const dmgFactor =
            typeof GAME_STATE !== 'undefined' && GAME_STATE.dmgFactor
    ...
    update(now) {
        ...
        if (
            ...
            if (ranked.length > 0) {
                ...
                if (
                    this.player.rocketEnabled &&
                    Math.random() <
                        GAME_CONFIG.UPGRADES.ROCKET_PLAYER_CHANCE_PCT / 100
                ) {
                ...
                if (
                    ...
                    this.volleyCount %
                        GAME_CONFIG.UPGRADES.SNIPER_SHOT_INTERVAL ===
                        0
                ) {
                    const strongest = Unit.findStrongestClosest(
                        this.player.x,
                        this.player.y,
                    );
                    if (strongest) {
                        const angle = Math.atan2(
                            strongest.y - this.player.y,
                            strongest.x - this.player.x,
                        );
                        const mmDmg = this.getDamage();
                        const mult = this.player.laserSniperEnabled
                            ? GAME_CONFIG.UPGRADES.LASER_SNIPER_DAMAGE_MULT
                            : GAME_CONFIG.UPGRADES.SNIPER_SHOT_DAMAGE_MULT;
        ...
        let aoeRadius = 0;
        if (this.player?.clusterShotEnabled) {
            const baseMineDmg = 18;
            dmg =
                baseMineDmg *
                0.5 *
                this.player.mineDamageModifier *
                GAME_STATE.dmgFactor;
            isExplosive = true;
            aoeRadius =
                (50 / 3) *
                this.player.mineAoeModifier *
        ...
        const spread =
            this.baseSpread *
            (this.player ? this.player.accuracyModifier : 1.0);
        ...
        if (this.player?.instantMissileEnabled) {
            ...
            const effSpeed =
                this.speed *
                (this.player && this.player.accuracyModifier === 0 ? 1.5 : 1.0);
            const vx = Math.cos(angle) * effSpeed;
            const vy = Math.sin(angle) * effSpeed;
            const kind =
                this.player && this.player.accuracyModifier === 0
                    ? 'laser'
                    : 'missile';
...
class MeleeSweep extends Weapon {
    ...
    getRange() {
        const mod = this.player?.meleeRangeModifier
            ? this.player.meleeRangeModifier
            : 1.0;
    ...
    update(now) {
        ...
        if (this.player?.sledgeEnabled && now - this.lastSledgeFire >= cd * 2) {
            ...
            const dmg = this.getDamage();
            const sledgeDmg =
                dmg * (GAME_CONFIG.UPGRADES.MELEE_SLEDGE_DAMAGE_PCT / 100);
            const radius =
                100 *
                this.player.meleeRangeModifier *
                ((GAME_STATE.difficulty
                ...
                SPATIAL_GRID.queryBox(
                    ...
                    (e) => {
                        ...
                        const hitHandle =
                            rx >= 0 &&
                            rx <= handleLength &&
                            Math.abs(ry) <= handleW / 2 + e.r;
                        // 2. Collision with wide bulbous head
                        const hitHead =
                            rx >= handleLength &&
                            rx <= radius + e.r &&
                            Math.abs(ry) <= headWidth / 2 + e.r;
...
class ProximityMine extends Weapon {
    ...
    getCooldown() {
        return (
            this.baseCooldown *
            (this.player ? this.player.mineCooldownModifier : 1.0)
        );
    }
    ...
    update(now) {
        ...
        if (now - this.lastFire >= cd && GAME_STATE.enemies.length > 0) {
            ...
            if (
                (this.player.mineAoeCount || 0) > 0 ||
                this.player.scatterMinesEnabled
            ) {
            ...
            if (this.player.mineThrowEnabled) {
                ...
                const speed = 12.0;
                const tsx =
                    this.player.x + Math.cos(angle) * (this.player.r - 2);
                const tsy =
                    this.player.y + Math.sin(angle) * (this.player.r - 2);
                const thrownMine = new PlayerMine(
                    tsx,
                    tsy,
                    8,
                    this.damage,
            ...
            if (
                this.player.scatterMinesEnabled &&
                Math.random() <
                    GAME_CONFIG.UPGRADES.MINE_SCATTER_CHANCE_PCT / 100
            ) {
                const count =
                    GAME_CONFIG.UPGRADES.MINE_SCATTER_MIN +
                    Math.floor(
                        Math.random() *
                            (GAME_CONFIG.UPGRADES.MINE_SCATTER_MAX -
                                GAME_CONFIG.UPGRADES.MINE_SCATTER_MIN +
                                1),
                    );
                for (let i = 0; i < count; i++) {
                    ...
                    const extraMine = new PlayerMine(
                        this.player.x,
                        this.player.y,
                        8,
                        this.damage,
...
class TurretWeapon extends Weapon {
    constructor(player) {
        super(
            player,
            'turret',
            GAME_CONFIG.TURRET.PLACEMENT_INTERVAL_SEC * 1000,
            0,
        );
    }
    ...
    getCooldown() {
        return (
            this.baseCooldown *
            (this.player ? this.player.buildingCooldownModifier : 1.0)
        );
    }
...
class DeflectorShields extends Weapon {
    ...
    update(now) {
        if (!this.initialized) {
            ...
            for (let i = 0; i < count; i++) {
                const angle = (i / count) * Math.PI * 2;
                const orb = new DeflectorOrbiter(
                    this.player,
                    angle,
                    this.rotSpeed,
                    this.radius,
...
class PlayerFlail extends Weapon {
    ...
    update(now) {
        ...
        this.r =
            this.baseR * (this.player.meleeRangeModifier > 1.0 ? 1.5 : 1.0);
    ...
    draw(now = performance.now()) {
        ...
        if (dist > 5) {
            ...
            const swingLag = Math.min(
                15,
                Math.max(-15, (this.vx * -normalY + this.vy * normalX) * 1.2),
            );
            ...
            for (let i = 0; i <= segments; i++) {
                ...
                const omt = 1 - t;
                const px =
                    omt * omt * this.player.x +
                    2 * omt * t * midX +
                    t * t * this.x;
                const py =
                    omt * omt * this.player.y +
                    2 * omt * t * midY +
                    t * t * this.y;

                // Segment radius: tapers organically and slenderly from root to tip
                const nodePulse =
                    Math.sin(t * Math.PI * segments * 0.75) * 0.35;
                const segR = Math.max(
                    1.6,
                    (1 - t) * (this.player.r * 0.22) +
                        t * 2.6 +
                        nodePulse * (1 - t * 0.5),
                );
            ...
            for (let i = 0; i < spinePoints.length; i++) {
                ...
                const ripple =
                    Math.sin(now * 0.007 - p.t * Math.PI * 3.5) *
                    (0.9 * (1 - p.t * 0.5));
            ...
            ctx.lineTo(
                leftContour[leftContour.length - 1].x,
                leftContour[leftContour.length - 1].y,
            );
            ctx.lineTo(
                rightContour[rightContour.length - 1].x,
                rightContour[rightContour.length - 1].y,
            );
        ...
        if ((this.player.freezeEnabled && speed > 5) || speed > 1.2) {
            const auraCol =
                this.player.freezeEnabled && speed > 5
                    ? '#00f0ff'
                    : this.player.color;
        ...
        ctx.beginPath();
        ctx.arc(-this.r * 0.32, -this.r * 0.32, this.r * 0.3, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (speed > 4.0) {
            ...
            ctx.arc(
                Math.cos(gleamAng) * (this.r + 6.5),
                Math.sin(gleamAng) * (this.r + 6.5),
                2.5,
                0,
                Math.PI * 2,
            );
...
function fireInstantMissile(
    ...
    const particleColor =
        player && player.accuracyModifier === 0
            ? player
                ? player.color
                : '#33ccff'
            : player
    ...
    const shootAngle = Math.atan2(
        targetEnemy.y - sourceY,
        targetEnemy.x - sourceX,
    );
    ...
    const shieldCheck =
        typeof findShieldArcIntersection === 'function'
    ...
    if (shieldCheck.blocked) {
        ...
        const hx = shieldCheck.hitX,
            hy = shieldCheck.hitY;
        if (isExplosive) {
            GAME_STATE.hazards.push(
                new MineExplosion(hx, hy, aoeRadius, now, player),
            );
        } else if (typeof spawnHitParticles === 'function') {
            spawnHitParticles(
                hx,
                hy,
                shieldCheck.isWallObstacle ? '#a8a29e' : '#ff8f00',
            );
        }
        return;
    }
    ...
    const hitAngle = Math.atan2(
        sourceY - targetEnemy.y,
        sourceX - targetEnemy.x,
    );
    ...
    if (isExplosive) {
        GAME_STATE.hazards.push(
            new MineExplosion(edgeX, edgeY, aoeRadius, now, player),
        );
        ...
        targetEnemy.hp -= dmg;
        if (player?.projectileLifedrainEnabled && sourceUnitType === 'player') {
            const healRatio =
                (GAME_CONFIG.UPGRADES.PROJECTILE_LIFEDRAIN_PCT || 10) / 100;
        ...
        if (player?.freezeEnabled && !targetEnemy.isBoss()) {
    ...
    if (player?.buckshotEnabled && !sourceTurret) {
        ...
        if (player.clusterShotEnabled) {
            ...
            buckshotDmg =
                (baseMineDmg / 3) *
                player.mineDamageModifier *
                GAME_STATE.dmgFactor;
            buckshotExplosive = true;
            buckshotAoe =
                (50 / 3) *
                player.mineAoeModifier *
                ((GAME_STATE.difficulty
                    ? GAME_STATE.difficulty.difficultyMultiplier || 1.0
                    : 1.0) /
                    2 +
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const sProj = new ShrapnelProjectile(
                sx,
                sy,
                svx,
                svy,
...
function updateLaserFences(dt, now) {
    ...
    for (let i = 0; i < numTurrets; i++) {
        ...
        if (
            !tur.player ||
            (!tur.player.laserWallsEnabled && !tur.player.slowWallsEnabled)
        )
            continue;
        ...
        for (let c = 0; c < numConns; c++) {
            ...
            if (tur.x < conn.x || (tur.x === conn.x && tur.y < conn.y)) {
                ...
                const isDamaging = !!tur.player?.laserWallsEnabled;
                const dmgPerSec = isDamaging
                    ? GAME_CONFIG.TURRET.LASER_WALL_DPS *
                      tur.player.damageModifier *
                      GAME_STATE.dmgFactor
                    : 0;
        ...
        for (let i = 0; i < numEnemies; i++) {
            ...
            if (edx * edx + edy * edy < hitR * hitR) {
                ...
                if (isDamaging && dmgThisFrame > 0) {
                    e.hp -= dmgThisFrame;
                    if (
                        !e.lastLaserFenceParticle ||
                        now >= e.lastLaserFenceParticle
                    ) {
