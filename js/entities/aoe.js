            ...
            for (const p of GAME_STATE.players) {
                ...
                if (dx * dx + dy * dy < (this.r + p.r) * (this.r + p.r)) {
                    if (!this.hitCooldown.has(p)) {
                        ...
                        if (curTime >= nextHit) {
                            if (p.takeDamage)
                                p.takeDamage(burnTick, curTime, this);
                            this.hitCooldown.set(p, curTime + 133.3);
                        }
                    }
                } else {
            ...
            for (const t of GAME_STATE.turrets) {
                ...
                if (dx * dx + dy * dy < (this.r + t.r) * (this.r + t.r)) {
                    if (!this.hitCooldown.has(t)) {
                        ...
                        if (curTime >= nextHit) {
                            if (t.takeDamage)
                                t.takeDamage(burnTick, curTime, this);
                            this.hitCooldown.set(t, curTime + 133.3);
                        }
                    }
                } else {
...
class BurningTrailSegment extends Entity {
    ...
    update(dt, now) {
        ...
        const dmg =
            this.damage *
            (this.player ? this.player.damageModifier : 1.0) *
            dmgFactor;
    ...
    update(dt, now) {
        ...
        if (typeof SPATIAL_GRID !== 'undefined' && SPATIAL_GRID.queryBox) {
            SPATIAL_GRID.queryBox(
                ...
                this.maxY + 85,
                (e) => {
                    if (typeof isDamageable === 'function' && !isDamageable(e))
                        return;
                    if (!e || e.hp <= 0) return;
                        ...
                        if (curTime >= nextHit) {
                            ...
                            if (typeof spawnHitParticles === 'function')
                                spawnHitParticles(
                                    e.x,
                                    e.y,
                                    this.player ? this.player.color : '#33ccff',
                                );
                        }
                    }
                },
...
function drawCryoMineFrost(ctx, x, y, r, now) {
    ...
    for (let i = 0; i <= segs; i++) {
        ...
        const nz =
            r * 0.1 * Math.sin(a * 3 + now * 0.0035 + 2.1) +
            r * 0.08 * Math.sin(a * 8 + now * 0.005 + 0.4);
        const rr = r * 1.22 + nz;
    ...
    for (let i = 0; i < plumeCount; i++) {
        const baseAng =
            (i / plumeCount) * Math.PI * 2 + (i % 2) * 0.28 + now * 0.0004;
        ...
        for (let k = 0; k < steps; k++) {
            ...
            if (hf < 0) continue;
            const sway =
                Math.sin(hf * 3.0 + now * 0.006 + i * 2.4) * (r * 0.6 * hf);
            const px = ox + sway;
...
function drawBioMineVesicle(
    ...
    if (isTriggered) {
        ...
        const flash =
            Math.floor(curTime / Math.max(40, 220 - 180 * progress)) % 2 === 0;
        coreColor = flash ? '#ff2200' : '#ff8800';
        coreGlow = flash ? 10 : 4;
    } else {
        const glowPulse = 0.5 + 0.5 * Math.sin(curTime * 0.006);
        coreColor =
            glowPulse > 0.5 ? (player ? player.color : '#aaff00') : '#99802f';
    }
    ...
    if (coreGlow > 0) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, curR * (isTriggered ? 0.7 : 0.55), 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.fillStyle = coreColor;
    ...
    ctx.arc(
        x,
        y,
        curR * (isTriggered ? 0.55 : 0.42 + stks * 0.015),
        0,
        Math.PI * 2,
    );
    ctx.fill();
...
class PlayerMine extends Entity {
    constructor(x, y, r, damage, player, now) {
        ...
        this.explosionRadius =
            50 * (player ? player.mineAoeModifier : 1.0) * (diffMult / 2 + 0.5);
        this.powderStacks = player ? player.mineAoeCount || 0 : 0;
        const attractChance =
            player?.mineAttractEnabled && typeof GAME_CONFIG !== 'undefined'
                ? GAME_CONFIG.UPGRADES.MINE_ATTRACT_CHANCE_PCT / 100
                : 0;
    ...
    static enforceCap() {
        const maxMines =
            (typeof GAME_CONFIG !== 'undefined' &&
                ...
                GAME_CONFIG.UPGRADES.MAX_ACTIVE_MINES) ||
            200;
    ...
    update(dt, now) {
        ...
        if (this.triggeredTime > 0) {
            const attractDurationSec =
                typeof GAME_CONFIG !== 'undefined' && GAME_CONFIG.UPGRADES
                    ? GAME_CONFIG.UPGRADES.MINE_ATTRACT_DURATION_SEC || 2.5
                    : 2.5;
        ...
        if (typeof SPATIAL_GRID !== 'undefined' && SPATIAL_GRID.queryBox) {
            SPATIAL_GRID.queryBox(
                ...
                this.y + mBox,
                (e) => {
                    if (typeof isTargetable === 'function' && !isTargetable(e))
                        return;
                    const dx = e.x - this.x;
                    const dy = e.y - this.y;
    ...
    explode(now) {
        ...
        const dmg =
            this.damage *
            (this.player ? this.player.mineDamageModifier : 1.0) *
            dmgFactor;
        let totalMineDmg = 0;
        ...
        if (typeof SPATIAL_GRID !== 'undefined' && SPATIAL_GRID.queryBox) {
            SPATIAL_GRID.queryBox(
                ...
                (e) => {
                    if (
                        hitSet.has(e) ||
                        (typeof isDamageable === 'function' && !isDamageable(e))
                    )
                        return;
                    const dx = e.x - this.x;
                    const dy = e.y - this.y;
        ...
        if (
            ...
            for (let i = 0; i < sporeCount; i++) {
                ...
                GAME_STATE.particles.push(
                    new Particle(
                        this.x + Math.cos(spAng) * spDist,
                        this.y + Math.sin(spAng) * spDist,
                        Math.cos(spAng) * spSpd,
                        Math.sin(spAng) * spSpd,
                        '#ffd54f',
                        400 + Math.random() * 200,
                    ),
                );
        ...
        if (
            typeof GAME_STATE !== 'undefined' &&
            GAME_STATE.hazards &&
            typeof MineExplosion !== 'undefined'
        ) {
...
class AcidPoolHazard extends Entity {
    ...
    draw(now) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            ctx.beginPath();
            ctx.arc(
                this.x + Math.cos(bAngle) * bDist,
                this.y + Math.sin(bAngle) * bDist,
                3,
                0,
                Math.PI * 2,
            );
            ...
            if (typeof GAME_STATE !== 'undefined' && GAME_STATE.players) {
                for (const p of GAME_STATE.players) {
                    ...
                    if (dx * dx + dy * dy <= maxDist * maxDist) {
                        if (p.takeDamage)
                            p.takeDamage(35, curTime, { type: 'behemoth' });
                    }
                }
            }
            if (
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (spiralR > 2) {
                ...
                ctx.arc(
                    ...
                    this.y,
                    spiralR,
                    spinAngle + i * Math.PI * 0.5,
                    spinAngle + (i + 1.2) * Math.PI * 0.5,
                );
                ctx.stroke();
            }
        }
...
function applyExplosionHealing(
    x,
    y,
    radius,
    totalDmg,
    ...
    if (!causingPlayer?.explosionHealEnabled || totalDmg <= 0) return;
    const healPct =
        typeof GAME_CONFIG !== 'undefined' && GAME_CONFIG.UPGRADES
            ? GAME_CONFIG.UPGRADES.EXPLOSION_HEAL_PCT || 10
            : 10;
    ...
    if (typeof GAME_STATE !== 'undefined' && GAME_STATE.players) {
        for (const p of GAME_STATE.players) {
            ...
            if (dx * dx + dy * dy <= (radius + p.r) * (radius + p.r)) {
                ...
                if (typeof spawnHitParticles === 'function')
                    spawnHitParticles(p.x, p.y, '#00ffcc');
                healedPlayers.push(p);
            }
        }
    }
    if (
        healedPlayers.length > 0 &&
        typeof GAME_STATE !== 'undefined' &&
        GAME_STATE.particles &&
...
function triggerWarpAnomalyDeathEffect(x, y, radius = 320, now) {
        ...
        for (const p of GAME_STATE.players) {
            ...
            if (dist <= radius && dist > 0.001) {
                ...
                p.knockbackTargetX = Math.max(
                    20,
                    Math.min(curW - 20, p.x + nx * impulse),
                );
                p.knockbackTargetY = Math.max(
                    20,
                    Math.min(curH - 20, p.y + ny * impulse),
                );
    ...
    if (typeof GAME_STATE !== 'undefined' && GAME_STATE.enemies) {
        ...
        if (typeof SPATIAL_GRID !== 'undefined' && SPATIAL_GRID.queryBox) {
            SPATIAL_GRID.queryBox(
                ...
                (e) => {
                    if (
                        affectedEnemies.has(e) ||
                        (typeof isTargetable === 'function' && !isTargetable(e))
                    )
                        return;
                    const dx = e.x - x;
                    const dy = e.y - y;
                    ...
                    if (dist <= radius && dist > 0.001) {
                        ...
                        e.knockbackTargetX = Math.max(
                            10,
                            Math.min(curW - 10, e.x + nx * impulse),
                        );
                        e.knockbackTargetY = Math.max(
                            10,
                            Math.min(curH - 10, e.y + ny * impulse),
                        );
    ...
    if (typeof GAME_STATE !== 'undefined' && GAME_STATE.hazards) {
        if (isPush) {
            ...
            if (
                typeof GAME_STATE.particles !== 'undefined' &&
                typeof Particle !== 'undefined'
            ) {
                for (let i = 0; i < 65; i++) {
...
function triggerFullBoardMineExplosion(now) {
    ...
    if (
        typeof GAME_STATE !== 'undefined' &&
        GAME_STATE.hazards &&
        typeof NukeExplosion !== 'undefined'
    ) {
        GAME_STATE.hazards.push(
            new NukeExplosion(curW / 2, curH / 2, maxDim * 0.85, curTime),
        );
    }
    ...
    const nukeDamage =
        180 * (typeof GAME_STATE !== 'undefined' ? GAME_STATE.dmgFactor : 1.0);
    if (typeof GAME_STATE !== 'undefined' && GAME_STATE.enemies) {
        for (const e of GAME_STATE.enemies) {
            if (typeof isDamageable === 'function' && !isDamageable(e))
                continue;
