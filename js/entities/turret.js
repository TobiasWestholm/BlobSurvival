class TurretEntity extends Unit {
    constructor(x, y, player, now) {
        ...
        this.flameDamage =
            GAME_CONFIG.TURRET.BASE_DAMAGE *
            GAME_CONFIG.TURRET.FLAME_DAMAGE_MULT;
        ...
        if (
            this.player &&
            (this.player.laserWallsEnabled || this.player.slowWallsEnabled)
        ) {
            this.linkWalls();
        }
    }
    get flameRange() {
        return this.player?.turretFullSweepEnabled
            ? GAME_CONFIG.TURRET.FLAME_SWEEP_RANGE
            : GAME_CONFIG.TURRET.FLAME_BASE_RANGE;
    }
    get flameConeWidth() {
        return this.player?.turretFullSweepEnabled
            ? Math.PI * 2
            : (GAME_CONFIG.TURRET.FLAME_BASE_CONE_DEG * Math.PI) / 180;
    }
    ...
    linkWalls() {
        ...
        if (candidates.length === 1) {
            ...
            const zeroConnIndex = candidates.findIndex(
                (c) => c.turret.connections.length === 0,
            );
            if (zeroConnIndex !== -1) {
                const p1 = candidates[zeroConnIndex].turret;
                const remaining = candidates.filter(
                    (_, idx) => idx !== zeroConnIndex,
                );
    ...
    dispenseItem(now) {
        ...
        this.lastDispenseColor =
            type === 'health'
                ? '#ff3366'
                : type === 'xp_cluster'
                  ? '#00ffcc'
                  : '#ffff00';
        this.dispenseFlashUntil =
            (typeof now === 'number' ? now : gameClock) + 260;
        ...
        if (type === 'health') {
            ...
            XPGem.createXPGems(
                this.x,
                this.y,
                GAME_CONFIG.SUPPLIES.XP_CLUSTER_XP,
            );
    ...
    takeDamage(amount, now, source) {
        ...
        if (source) {
            ...
            const cooldownMs =
                source instanceof Enemy && source.isPassingThroughLaserFence()
                    ? 500
                    : 200;
    ...
    onTakeDamage(amount, now, source) {
        ...
        if (
            this.player?.reflectDamageEnabled &&
            reflectTarget &&
            ...
            const reflectDmg =
                this.maxHp *
                (GAME_CONFIG.UPGRADES.REFLECT_DAMAGE_TURRET_MAX_HP_PCT / 100);
    ...
    update(dt, dtFactor, now) {
        ...
        if (this.player?.turretSawEnabled) {
            ...
            const sawRadius =
                ...
                ((GAME_STATE.difficulty
                    ...
                    0.5);
            const dmgPerSec =
                GAME_CONFIG.TURRET.SAW_DPS *
                this.player.damageModifier *
            ...
            SPATIAL_GRID.queryBox(
                ...
                (e) => {
                    ...
                    if (
                        dx * dx + dy * dy <=
                        (sawRadius + e.r) * (sawRadius + e.r)
                    ) {
                        e.hp -= dmgThisFrame;
                        const nextParticleTime =
                            this.sawHitCooldown.get(e) || 0;
        ...
        if (this.player?.turretDispenserEnabled) {
            if (!this.lastDispenseCheck) this.lastDispenseCheck = now;
            const dispenserInterval =
                GAME_CONFIG.TURRET.DISPENSER_INTERVAL_SEC *
                1000 *
            ...
            if (now - this.lastDispenseCheck >= dispenserInterval) {
                ...
                if (
                    Math.random() <
                    GAME_CONFIG.TURRET.DISPENSER_CHANCE_PCT / 100
                ) {
        ...
        if (now - this.lastFire >= attackCd && GAME_STATE.enemies.length > 0) {
            ...
            if (closestMissile) {
                ...
                const targetAngle = Math.atan2(
                    closestMissile.y - this.y,
                    closestMissile.x - this.x,
                );
                ...
                if (this.player?.instantMissileEnabled) {
                    fireInstantMissile(
                        this.x,
                        this.y,
                        closestMissile,
                    ...
                    const spreadAngle =
                        ...
                        (Math.random() - 0.5) *
                            this.baseSpread *
                            this.player.accuracyModifier;
                    const kind =
                        this.player.accuracyModifier === 0
                            ? 'laser'
                            : 'missile';
                    const effSpeed =
                        this.projectileSpeed *
                        (this.player.accuracyModifier === 0 ? 1.5 : 1.0);
                ...
                if (this.player?.rocketEnabled && Math.random() < 0.05) {
                    const rAngle = Math.random() * Math.PI * 2;
                    const rDmg =
                        30 * this.player.damageModifier * GAME_STATE.dmgFactor;
                ...
                if (
                    this.player?.sniperShotEnabled &&
                    this.attackCount % 4 === 0
                ) {
                    let strongest = null;
            ...
            if (this.isFlamethrower) {
                const closestFlame = Unit.findClosest(
                    this,
                    GAME_STATE.enemies,
                    this.flameRange,
                    isTargetable,
                ...
                if (closestFlame) {
                    const flameTargetAngle = Math.atan2(
                        closestFlame.y - this.y,
                        closestFlame.x - this.x,
                    );
    ...
    draw(now) {
        ...
        if (now < this.flameActiveUntil) {
            ...
            const fRange = this.flameRange;
            const coneHalf = this.flameConeWidth / 2;
            const currentSweepAngle =
                this.flameCenterAngle - coneHalf + coneHalf * 2 * t;
            ...
            if (coneHalf >= Math.PI) {
                ...
                ctx.moveTo(
                    this.x + Math.cos(this.flameCenterAngle - coneHalf) * 18,
                    this.y + Math.sin(this.flameCenterAngle - coneHalf) * 18,
                );
                ctx.arc(
                    this.x,
                    this.y,
                    fRange,
                    this.flameCenterAngle - coneHalf,
                    this.flameCenterAngle + coneHalf,
                );
                ctx.lineTo(
                    this.x + Math.cos(this.flameCenterAngle + coneHalf) * 18,
            ...
            ctx.beginPath();
            ctx.moveTo(
                this.x + Math.cos(currentSweepAngle) * 18,
                this.y + Math.sin(currentSweepAngle) * 18,
            );
            ctx.lineTo(
                this.x + Math.cos(currentSweepAngle) * fRange,
                this.y + Math.sin(currentSweepAngle) * fRange,
            );
            ctx.stroke();
            ctx.restore();
        }
        ...
        if (this.pendingExpansion) {
            ...
            for (const root of roots) {
                ...
                ctx.moveTo(x0, y0);
                ctx.lineTo(
                    x0 + Math.cos(root.angle) * curDist,
                    y0 + Math.sin(root.angle) * curDist,
                );
            }
            ctx.stroke();
                ...
                for (let s = 1; s <= segments; s++) {
                    ...
                    const lateralOffset =
                        s === segments
                            ? 0
                            : Math.sin(
                                  segDist * root.wiggleFreq +
                                      root.wiggleSeed +
                                      now * 0.005,
                              ) * 1.8;
                    const px =
                        x0 +
                        Math.cos(root.angle) * segDist -
                        Math.sin(root.angle) * lateralOffset;
                    const py =
                        y0 +
                        Math.sin(root.angle) * segDist +
                        Math.cos(root.angle) * lateralOffset;
                    ctx.lineTo(px, py);
                }
            }
            ctx.strokeStyle = pColor;
            ...
            if (t >= 0.35) {
                ...
                ctx.fillStyle = '#ffffff';
                ctx.globalAlpha = 0.4 * destIntensity;
                ctx.beginPath();
                for (let v = 0; v < 4; v++) {
        ...
        for (const conn of this.connections) {
            ...
            if (this.x < conn.x || (this.x === conn.x && this.y < conn.y)) {
                ...
                const fromTurret =
                    this.spawnTime <= conn.spawnTime ? this : conn;
                ...
                const midY =
                    (y0 + y1) * 0.5 +
                    Math.sin(now * 0.004 + fromTurret.x * 0.04) * 2;
                ...
                if (isEstablishing) {
                    ...
                    const tRaw = Math.max(
                        0,
                        Math.min(1, elapsed / animDuration),
                    );
                    ...
                    ctx.lineWidth =
                        (isLaserWall ? 4.0 : 2.5) + (isLaserWall ? 8 : 5);
                    ...
                    ctx.strokeStyle = fenceColor;
                    ctx.lineWidth =
                        (isLaserWall ? 3.0 : 1.8) + (isLaserWall ? 6 : 4);
                    ctx.globalAlpha = (isLaserWall ? 0.7 : 0.45) * pulse * 0.4;
                    ctx.beginPath();
                    ctx.moveTo(x0, y0);
                    ...
                    ctx.globalAlpha = (isLaserWall ? 0.7 : 0.45) * pulse;
                    ctx.beginPath();
                    ctx.moveTo(x0, y0);
                    ctx.quadraticCurveTo(midX, midY, x1, y1);
                    ctx.stroke();
        ...
        if (this.player?.alive) {
            ...
            if (timeSinceSpawn < 320) {
                ...
                if (pdist > 0.01 && pdist < 120) {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(
                        this.player.x + normX * (this.player.r * 0.4),
                        this.player.y + normY * (this.player.r * 0.4),
                    );
                    ctx.quadraticCurveTo(
                        midX + normX * pinch,
                        midY + normY * pinch,
                        attachTurretX + normX * 1.5,
                        attachTurretY + normY * 1.5,
                    );
                    ctx.lineTo(
                        attachTurretX - normX * 1.5,
                        attachTurretY - normY * 1.5,
                    ...
                    ctx.strokeStyle = '#ffffff';
                    ctx.lineWidth = 1.2;
                    ctx.globalAlpha = tetherAlpha * 0.5;
                    ctx.beginPath();
                    ctx.arc(
                        this.x,
                        this.y,
                ...
                if (rc > 0) {
                    ...
                    for (let i = 0; i < moteCount; i++) {
                        ...
                        ctx.beginPath();
                        ctx.arc(
                            this.x + Math.cos(ma) * md,
                            this.y + Math.sin(ma) * md,
                            1.1,
                            0,
                            Math.PI * 2,
                        );
        ...
        if (this.player?.turretSawEnabled) {
            ...
            const darkSawFill = shadeHex(pColor, 0.1);
        ...
        const steelTint = shadeHex(pColor, 1.5);
        if (dCount > 0) {
            const collarR = 7.0;
            const merlonH = 1.9;
            ...
            for (let i = 0; i < teeth; i++) {
                ...
                ctx.lineTo(
                    Math.cos(a0) * (collarR + merlonH),
                    Math.sin(a0) * (collarR + merlonH),
                );
                ctx.lineTo(
                    Math.cos(a1) * (collarR + merlonH),
                    Math.sin(a1) * (collarR + merlonH),
                );
        ...
        for (let i = 0; i < dendriteCount; i++) {
            ...
            const baseReach =
                ...
                1.6 *
                    Math.sin(
                        (isOverclocked ? now * 0.01 : now * 0.003) + i * 1.8,
                    );
        ...
        const isInfernoNova = this.player?.turretFullSweepEnabled;
        if (isInfernoNova) {
            const grad = ctx.createRadialGradient(0, 0, 1.0, 0, 0, 11.5);
            grad.addColorStop(0, '#ffffff');
            grad.addColorStop(0.2, pColor);
            grad.addColorStop(0.7, pColor);
        ...
        if (isInfernoNova) {
            ...
            for (let i = 0; i < dendriteCount; i++) {
                const baseAngle =
                    (i / dendriteCount) * Math.PI * 2 +
                    Math.sin(now * 0.003 + i * 1.5) * 0.08;
                const baseReach =
                    9.2 + dCount * 0.5 + 1.6 * Math.sin(now * 0.004 + i * 1.8);
                const tipX = Math.cos(baseAngle) * baseReach;
                const tipY = Math.sin(baseAngle) * baseReach;
                const midX =
                    tipX * 0.55 +
                    Math.cos(baseAngle + Math.PI / 2) *
                        (1.2 * Math.sin(now * 0.008 + i));
                const midY =
                    tipY * 0.55 +
                ...
                ctx.moveTo(
                    Math.cos(baseAngle) * 3.5,
                    Math.sin(baseAngle) * 3.5,
                );
            ...
            ctx.strokeStyle = isOverclocked
                ...
                : this.player
                  ...
                  : '#333';
            ctx.lineWidth = 0.9;
            ctx.globalAlpha = isOverclocked ? 0.75 : 0.4;
            ctx.beginPath();
            for (let i = 0; i < dendriteCount; i++) {
                const baseAngle =
                    (i / dendriteCount) * Math.PI * 2 +
                    Math.sin(
                        (isOverclocked ? now * 0.009 : now * 0.0025) + i * 1.5,
                    ) *
                        (isOverclocked ? 0.14 : 0.08);
                const baseReach =
                    9.2 +
                    dCount * 0.5 +
                    1.6 *
                        Math.sin(
                            (isOverclocked ? now * 0.01 : now * 0.003) +
                                i * 1.8,
                        );
                const tipX = Math.cos(baseAngle) * baseReach;
                const tipY = Math.sin(baseAngle) * baseReach;
                ...
                ctx.lineTo(
                    tipX * 0.65 + Math.cos(b1Angle) * 3.2,
                    tipY * 0.65 + Math.sin(b1Angle) * 3.2,
                );
            }
            ctx.stroke();
        }
        ...
        if (this.player?.turretDispenserEnabled) {
            ...
            const dProg = Math.min(
                1,
                Math.max(0, (now - lastCheck) / dInterval),
            );
            const pouchY = 7.6;
            const flash = this.dispenseFlashUntil > now;
            const flashT = flash
                ? Math.max(0, (this.dispenseFlashUntil - now) / 260)
                : 0;
            ...
            ctx.fillStyle = shadeHex(
                flash ? this.lastDispenseColor || '#ffff88' : '#ffff88',
                1.35,
            );
        ...
        if (isInfernoNova) {
            ...
            const nucleusPulse = isOverclocked
                ? 0.75 + 0.25 * Math.sin(now * 0.035)
                : 0.5 + 0.3 * Math.sin(now * 0.005);
        ...
        if (this.isFlamethrower) {
            if (isInfernoNova) {
                ...
                const flareT = isFiringNova
                    ? Math.max(0, (this.flameActiveUntil - now) / 400)
                    : 0;
                ...
                for (let v = 0; v < 4; v++) {
                    ...
                    if (isFiringNova) {
                        ...
                        bloom.addColorStop(0.7, 'rgba(255, 68, 0, 0.40)');
        ...
        if (this.player?.turretNetworkEnabled) {
            const connCount = this.connections
                ? this.connections.filter((c) => c.alive).length
                : 0;
            ...
            for (let i = 0; i < 2; i++) {
                ...
                const sockX = sockOffsets[i];
                const diodePulse = isLinked
                    ? 1.0
                    : 0.4 + 0.6 * Math.sin(now * 0.008 + i * 2.0);
            ...
            const beaconColor = isPreExpanding
                ? '#ffffff'
                : hasFreeLink
                  ? '#00ffff'
                  : '#ffaa00';
        ...
        if (isOverclocked && Math.random() < 0.1) {
        ...
        if (this.hp < this.maxHp) {
            ...
            ctx.fillStyle =
                hpPct > 0.5 ? '#00ffcc' : hpPct > 0.25 ? '#ffff00' : '#ff3344';
