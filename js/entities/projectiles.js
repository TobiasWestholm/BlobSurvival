class Projectile extends Entity {
    ...
    get player() {
        return typeof Player !== 'undefined' && this.owner instanceof Player
            ? this.owner
            : this.owner?.player
              ? this.owner.player
              : this.owner || null;
    ...
    checkBounds(
        minX = -50,
        minY = -50,
        maxX = typeof W !== 'undefined' ? W + 50 : 1562,
        maxY = typeof H !== 'undefined' ? H + 50 : 950,
    ) {
...
class RocketProjectile extends Projectile {
    ...
    detonate(now, hitObstacle = null) {
        ...
        const blastRadius =
            90 *
            (this.player ? this.player.mineAoeModifier : 1.0) *
            ((GAME_STATE.difficulty
                ...
                0.5);
        const dmg =
            this.damage * this.player.damageModifier * GAME_STATE.dmgFactor;
        GAME_STATE.hazards.push(
            new MineExplosion(this.x, this.y, blastRadius, now, this.player),
        );
            ...
            if (
                ...
                if (hitObstacle) {
                    if (hitObstacle.isWallObstacle) {
                        const hitCheck = testOrientedBoxHit(
                            this.x,
                            this.y,
                            e.x,
                            e.y,
                        ...
                        const sX = hitObstacle.x,
                            sY = hitObstacle.y;
                        const sR =
                            hitObstacle.shieldRadius || hitObstacle.r || 100;
                        const sFacing = hitObstacle.facingAngle || 0;
                        const sHalfArc =
                            hitObstacle.shieldHalfArc || Math.PI * 0.5;
                        const hitCheck = testShieldArcHit(
                            this.x,
                            this.y,
                            e.x,
                            e.y,
...
class SniperProjectile extends Projectile {
    ...
    update(dt, dtFactor = 1.0, now) {
        ...
        const shieldCheck = findShieldArcIntersection(
            prevX,
            prevY,
            nextX,
            nextY,
        ...
        if (shieldCheck?.blocked) {
            ...
            if (this.player?.laserSniperEnabled) {
                GAME_STATE.hazards.push(
...
class ShooterProjectile extends Projectile {
    ...
    update(dt, dtFactor = 1.0, now) {
        ...
        if (this.isExpired(now) || this.checkBounds(-40, -40, W + 40, H + 40))
            return;

        // Deflector Orbiters interception check (Shooter bolts)
        if (
            typeof checkDeflectorOrbiterBlock === 'function' &&
            checkDeflectorOrbiterBlock(this, now)
        )
            return;
        for (const p of GAME_STATE.players) {
            if (!p.alive) continue;
            ...
            if (dx * dx + dy * dy < (p.r + this.r) * (p.r + this.r)) {
                p.takeDamage(this.damage, now, this);
                this.despawn();
                const pColor = this.sourceEnemy?.color
                    ? brightenColor(this.sourceEnemy.color, 1.8)
                    : '#bb44ff';
    ...
    draw() {
        ...
        const baseColor = this.sourceEnemy?.color
            ? this.sourceEnemy.color
            : '#bb44ff';
...
class OrbitProjectile extends Projectile {
                    ...
                    if (this.player.mineRingEnabled) {
                        ...
                        if (now >= next) {
                            ...
                            const hitDelay = Math.max(
                                50,
                                400 * this.player.cooldownModifier,
                            );
    ...
    explode(now) {
        ...
        const dmg =
            baseDmg * this.player.mineDamageModifier * GAME_STATE.dmgFactor;
        const explosionRadius =
            ...
            ((GAME_STATE.difficulty
                ...
                0.5);
        const cooldown = pm
            ? pm.baseCooldown * this.player.mineCooldownModifier
            : 2400 * this.player.mineCooldownModifier;
        ...
        SPATIAL_GRID.queryBox(
            ...
            (e) => {
                ...
                if (
                    dx * dx + dy * dy <=
                    (explosionRadius + e.r) * (explosionRadius + e.r)
                ) {
                    hitSet.add(e);
                    e.hp -= dmg;
        ...
        if (this.player.mineRingEnabled) {
            ...
            if (this.respawnAnimation) {
                ...
                if (elapsed < this.respawnAnimation.duration) {
                    ...
                    const rootX =
                        this.player.x + Math.cos(this.angle) * this.player.r;
                    const rootY =
                        this.player.y + Math.sin(this.angle) * this.player.r;
                    const normX = -Math.sin(this.angle);
                    const normY = Math.cos(this.angle);

                    // Current reach distance along current orbiting angle
                    const currentDist =
                        this.player.r + (this.radius - this.player.r) * reach;
                    const tipX =
                        this.player.x + Math.cos(this.angle) * currentDist;
                    const tipY =
                        this.player.y + Math.sin(this.angle) * currentDist;
                    ...
                    if (reach > 0.02) {
                        ...
                        const midX =
                            this.player.x + Math.cos(this.angle) * midDist;
                        const midY =
                            this.player.y + Math.sin(this.angle) * midDist;
...
class DeflectorOrbiter extends Projectile {
    ...
    update(dt, dtFactor = 1.0, now) {
        ...
        const respawnMs =
            GAME_CONFIG.UPGRADES.DEFLECTOR_ORBITERS_RESPAWN_SEC * 1000;
        ...
        for (const proj of GAME_STATE.enemyProjectiles) {
            ...
            if (sweepLen2 > 0) {
                const t = Math.max(
                    ...
                    Math.min(
                        1,
                        ((proj.x - prevX) * dx_sweep +
                            (proj.y - prevY) * dy_sweep) /
                            sweepLen2,
                    ),
                );
...
class ShrapnelProjectile extends Projectile {
            ...
            SPATIAL_GRID.queryBox(
                ...
                (e) => {
                    ...
                    if (lenSq === 0) {
                        ...
                        const px = x1 + tClamped * dxSeg,
                            py = y1 + tClamped * dySeg;
                        const ex = e.x - px,
                            ey = e.y - py;
                        distSq = ex * ex + ey * ey;
                    }
    ...
    explode(now, hitTarget = null, hitObstacle = null) {
        ...
        const radius =
            this.aoeRadius ||
            18 *
                (this.player ? this.player.mineAoeModifier : 1) *
                ((GAME_STATE.difficulty
                    ? GAME_STATE.difficulty.difficultyMultiplier || 1.0
                    : 1.0) /
                    2 +
        ...
        GAME_STATE.hazards.push(
            new MineExplosion(this.x, this.y, radius, now, this.player),
        );
    }
    draw() {
        ctx.save();
...
class MagicMissileProjectile extends Projectile {
        ...
        if (bestEnemy) {
            if (this.isExplosive) {
                ...
                if (
                    ...
                    this.player.heal(PROJECTILE_HEAL * this.damage);
                    this.player.triggerLifestealVisual(
                        bestEnemy.x,
                        bestEnemy.y,
                    );
                }
                this.despawn();
                if (this.player?.freezeEnabled && !bestEnemy.isBoss()) {
                ...
                if (
                    this.player?.buckshotEnabled &&
                    (this.kind === 'missile' || this.kind === 'laser')
                ) {
    ...
    explode(now, hitTarget = null, hitObstacle = null) {
        ...
        if (
            ...
            this.spawnBuckshot(
                hitTarget || { x: this.x, y: this.y, r: 0 },
                now,
                hitObstacle,
            );
        }
            ...
            if (hitObstacle) {
                if (isExplosive) {
                    ...
                    spawnHitParticles(
                        fragX,
                        fragY,
                        hitObstacle.isWallObstacle ? '#a8a29e' : '#ff8f00',
                    );
                } else {
                    spawnHitParticles(
                        this.x + Math.cos(a) * 4,
                ...
                const proj = new ShrapnelProjectile(
                    sx,
                    sy,
                    vx,
                    vy,
