const UPGRADE_POOL = [
    {
        ...
        oneShot: true,
        effect: (p) => {
            p.speed *= 1 + GAME_CONFIG.UPGRADES.SPEED_BOOST_PCT / 100;
            p.agilityBoostEnabled = true;
        },
    },
    {
        id: 'speed_lvl2',
        name: 'Temporal Drift',
        desc: `Increase movement speed by another ${GAME_CONFIG.UPGRADES.SPEED_LVL2_BOOST_PCT}% and leave a damaging laser trail behind you.`,
        oneShot: true,
        effect: (p) => {
            p.speed *= 1 + GAME_CONFIG.UPGRADES.SPEED_LVL2_BOOST_PCT / 100;
            p.speedLvl2 = true;
        },
    },
    {
        id: 'damage',
        name: 'Heavy Impact',
        desc: `Increase all weapon damage by ${GAME_CONFIG.UPGRADES.DAMAGE_WEAPONS_BOOST_PCT}%, and proximity mines by ${GAME_CONFIG.UPGRADES.DAMAGE_MINES_BOOST_PCT}%.`,
        effect: (p) => {
            p.damageModifier *=
                1 + GAME_CONFIG.UPGRADES.DAMAGE_WEAPONS_BOOST_PCT / 100;
            p.mineDamageModifier *=
                1 + GAME_CONFIG.UPGRADES.DAMAGE_MINES_BOOST_PCT / 100;
            p.damageUpgradeCount = (p.damageUpgradeCount || 0) + 1;
        },
    },
    {
        id: 'cooldown',
        name: 'Hyper-drive',
        desc: `Increase attack speed by ${GAME_CONFIG.UPGRADES.ATTACK_SPEED_BOOST_PCT}%, and proximity mines by ${GAME_CONFIG.UPGRADES.MINE_COOLDOWN_BOOST_PCT}%.`,
        effect: (p) => {
            p.cooldownModifier /=
                1 + GAME_CONFIG.UPGRADES.ATTACK_SPEED_BOOST_PCT / 100;
            p.mineCooldownModifier /=
                1 + GAME_CONFIG.UPGRADES.MINE_COOLDOWN_BOOST_PCT / 100;
            p.attackSpeedCount = (p.attackSpeedCount || 0) + 1;
        },
    },
    {
        id: 'accuracy',
        name: 'Accurate Shot',
        desc: 'Reduce projectile spread to zero and increase projectile travel speed by 50%.',
        oneShot: true,
        effect: (p) => (p.accuracyModifier = 0),
    },
    {
        id: 'instant_missile_upgrade',
        name: 'Instant Precision',
        desc: 'Magic Missile, Laser, and Cluster Shot travel with infinite velocity, hitting monsters instantly without a traveling projectile.',
        oneShot: true,
        effect: (p) => (p.instantMissileEnabled = true),
    },
    {
        id: 'sniper_shot_upgrade',
        name: 'Sniper Shot',
        desc: `Every ${GAME_CONFIG.UPGRADES.SNIPER_SHOT_INTERVAL === 3 ? '3rd' : GAME_CONFIG.UPGRADES.SNIPER_SHOT_INTERVAL + 'th'} Magic Missile volley fires a high-speed piercing sniper shot at the strongest enemy, dealing ${GAME_CONFIG.UPGRADES.SNIPER_SHOT_DAMAGE_MULT}x damage.`,
        oneShot: true,
        effect: (p) => (p.sniperShotEnabled = true),
    },
    {
        id: 'laser_sniper_upgrade',
        name: 'Laser Sniper',
        desc: `Increases Sniper Shot damage to ${GAME_CONFIG.UPGRADES.LASER_SNIPER_DAMAGE_MULT}x Magic Missile and leaves a decaying laser trail along its path.`,
        oneShot: true,
        effect: (p) => (p.laserSniperEnabled = true),
    },
    {
        id: 'multishot',
        name: 'Split Shot',
        desc: 'Magic Missile fires at one additional enemy.',
        effect: (p) => (p.multiShot += 1),
    },
    {
        id: 'dash',
        name: 'Phase Dash',
        desc: 'Double-tap a move key to dash, then burst missiles around you.',
        oneShot: true,
        effect: (p) => (p.dashEnabled = true),
    },
    {
        id: 'dash_lvl2',
        name: 'Phase Mastery',
        desc: `Increases Phase Dash range by ${GAME_CONFIG.DASH.LVL2_RANGE_BOOST_PCT}%, recharge speed by ${(GAME_CONFIG.DASH.LVL2_COOLDOWN_DIVISOR - 1) * 100}%, leaves a burning fire trail when dashing, and grants invulnerability and a shield during the first half of recharge.`,
        oneShot: true,
        effect: (p) => {
            p.dashLvl2 = true;
            p.dashCooldown =
                PLAYER_DASH_COOLDOWN / GAME_CONFIG.DASH.LVL2_COOLDOWN_DIVISOR;
        },
    },
    {
        id: 'phase_detonation_upgrade',
        name: 'Phase Detonation',
        desc: 'Phase Dash landing triggers a full mine explosion and releases explosive magic missiles.',
        oneShot: true,
        effect: (p) => (p.phaseDetonationEnabled = true),
    },
    {
        id: 'unlock_ring',
        name: 'Fire Ring',
        desc: 'Unlock orbiting fire balls around you.',
        oneShot: true,
        effect: (p) => p.unlockWeapon('fire_ring'),
    },
    {
        id: 'unlock_missile',
        name: 'Magic Missile',
        desc: 'Unlock the auto-firing Magic Missile weapon.',
        oneShot: true,
        effect: (p) => p.unlockWeapon('magic_missile'),
    },
    {
        id: 'unlock_melee',
        name: 'Melee Sweep',
        desc: 'Unlock the close-range melee sweeping attack.',
        oneShot: true,
        effect: (p) => p.unlockWeapon('melee_sweep'),
    },
    {
        id: 'unlock_mine',
        name: 'Proximity Mine',
        desc: 'Unlock proximity mines that explode for AoE damage.',
        oneShot: true,
        effect: (p) => p.unlockWeapon('proximity_mine'),
    },
    {
        id: 'unlock_turret',
        name: 'Auto-Turret',
        desc: `Unlock deployable turrets that drop every ${GAME_CONFIG.TURRET.PLACEMENT_INTERVAL_SEC}s and shoot high-speed missiles for ${GAME_CONFIG.TURRET.LIFETIME_SEC}s.`,
        oneShot: true,
        effect: (p) => p.unlockWeapon('turret'),
    },
    {
        id: 'laser_walls_upgrade',
        name: 'Laser Fences',
        desc: `Turrets create high-energy laser barriers between them that deal ${GAME_CONFIG.TURRET.LASER_WALL_DPS} damage/sec to monsters passing through, while slowing them by ${GAME_CONFIG.TURRET.SLOW_WALL_SLOW_PCT}%.`,
        oneShot: true,
        effect: (p) => {
            p.laserWallsEnabled = true;
            p.slowWallsEnabled = true;
            for (const t of GAME_STATE.turrets) {
                if (t.alive && t.player === p) t.linkWalls();
            }
        },
    },
    {
        ...
        desc: `Increases the duration of turrets by ${GAME_CONFIG.TURRET.FORTIFIED_DURATION_BOOST_PCT}% and turret HP by ${GAME_CONFIG.TURRET.FORTIFIED_HP_BOOST_PCT}%.`,
        effect: (p) => {
            p.buildingDurationModifier *=
                1 + GAME_CONFIG.TURRET.FORTIFIED_DURATION_BOOST_PCT / 100;
            p.buildingHealthModifier *=
                1 + GAME_CONFIG.TURRET.FORTIFIED_HP_BOOST_PCT / 100;
    ...
    },
    {
        ...
        effect: (p) => {
            p.buildingCooldownModifier =
                (p.buildingCooldownModifier || 1.0) *
                (1 - GAME_CONFIG.TURRET.RAPID_DEPLOYMENT_REDUCTION_PCT / 100);
            p.turretCooldownCount = (p.turretCooldownCount || 0) + 1;
        },
    },
    {
        id: 'turret_flamethrower_upgrade',
        name: 'Flamethrower Turret',
        desc: `Adds a 2nd firing head to turrets that unleashes a ${GAME_CONFIG.TURRET.FLAME_BASE_RANGE}px ${GAME_CONFIG.TURRET.FLAME_BASE_CONE_DEG}° flame cone every ${GAME_CONFIG.TURRET.FLAME_INTERVAL_ATTACKS === 4 ? '4th' : GAME_CONFIG.TURRET.FLAME_INTERVAL_ATTACKS + 'th'} attack for ${GAME_CONFIG.TURRET.FLAME_DAMAGE_MULT}x damage.`,
        oneShot: true,
        effect: (p) => (p.turretFlamethrowerEnabled = true),
    },
    {
        id: 'turret_inferno_ring_upgrade',
        name: 'Inferno Nova',
        desc: `Flamethrower Turret sweeps a full 360° rotation around the turret and extends attack radius to ${GAME_CONFIG.TURRET.FLAME_SWEEP_RANGE}px.`,
        oneShot: true,
        effect: (p) => (p.turretFullSweepEnabled = true),
    },
    {
        id: 'turret_network_upgrade',
        name: 'Autonomous Network',
        desc: `Every ${GAME_CONFIG.TURRET.NETWORK_INTERVAL_SEC}s, turrets with free links spawn a new connected turret pointing away from existing links.`,
        oneShot: true,
        effect: (p) => (p.turretNetworkEnabled = true),
    },
    {
        id: 'turret_saw_upgrade',
        name: 'Sawblade Turrets',
        desc: `Equips turrets with rotating sawblades dealing continuous ${GAME_CONFIG.TURRET.SAW_DPS} damage/sec to all nearby monsters within ${GAME_CONFIG.TURRET.SAW_RADIUS}px.`,
        oneShot: true,
        effect: (p) => (p.turretSawEnabled = true),
    },
    {
        id: 'turret_dispenser_upgrade',
        name: 'Supply Dispenser',
        desc: `Each active turret has a ${GAME_CONFIG.TURRET.DISPENSER_CHANCE_PCT}% chance every ${GAME_CONFIG.TURRET.DISPENSER_INTERVAL_SEC}s to dispense power-up items.`,
        oneShot: true,
        effect: (p) => (p.turretDispenserEnabled = true),
    },
    {
        id: 'mine_aoe_upgrade',
        name: 'Volatile Powder',
        desc: `Increase proximity mine explosion radius by ${GAME_CONFIG.UPGRADES.MINE_AOE_BOOST_PCT}% and Martyrdom blast radius by ${GAME_CONFIG.UPGRADES.MARTYRDOM_AOE_BOOST_PCT}%.`,
        effect: (p) => {
            p.mineAoeModifier *=
                1 + GAME_CONFIG.UPGRADES.MINE_AOE_BOOST_PCT / 100;
            p.mineAoeCount = (p.mineAoeCount || 0) + 1;
        },
    },
    {
        id: 'mine_attract_upgrade',
        name: 'Magnetic Core',
        desc: `${GAME_CONFIG.UPGRADES.MINE_ATTRACT_CHANCE_PCT}% of proximity mines attract enemies for ${GAME_CONFIG.UPGRADES.MINE_ATTRACT_DURATION_SEC}s before exploding.`,
        oneShot: true,
        effect: (p) => (p.mineAttractEnabled = true),
    },
    {
        id: 'freeze_upgrade',
        name: 'Cryo Freeze',
        desc: `Magic Missiles and Scourge Flail freeze enemies for ${GAME_CONFIG.UPGRADES.FREEZE_PROJECTILE_DURATION_SEC}s, and mines freeze for ${GAME_CONFIG.UPGRADES.FREEZE_MINE_DURATION_SEC}s.`,
        oneShot: true,
        effect: (p) => (p.freezeEnabled = true),
    },
    {
        id: 'cryo_mine_upgrade',
        name: 'Polar Blast',
        desc: `Increase Proximity Mine and Scourge Flail freeze duration by ${GAME_CONFIG.UPGRADES.CRYO_MINE_BOOST_PCT}%.`,
        oneShot: true,
        effect: (p) => (p.cryoMineBuffed = true),
    },
    {
        id: 'mine_scatter_upgrade',
        name: 'Scatter Charges',
        desc: `${GAME_CONFIG.UPGRADES.MINE_SCATTER_CHANCE_PCT}% chance when dropping a mine to scatter ${GAME_CONFIG.UPGRADES.MINE_SCATTER_MIN}-${GAME_CONFIG.UPGRADES.MINE_SCATTER_MAX} extra mines in random directions around you.`,
        oneShot: true,
        effect: (p) => (p.scatterMinesEnabled = true),
    },
    {
        ...
        oneShot: true,
        effect: (p) => (p.mineThrowEnabled = true),
    },
    {
        id: 'explosion_heal_upgrade',
        name: 'Blast Mending',
        desc: `Explosions caused by you heal all players inside their blast radius equal to ${GAME_CONFIG.UPGRADES.EXPLOSION_HEAL_PCT}% of total damage dealt, and unlocks the Magnetic Core upgrade.`,
        oneShot: true,
        effect: (p) => (p.explosionHealEnabled = true),
    },
    {
        id: 'mine_ring',
        name: 'Explosive Ring',
        ...
        oneShot: true,
        effect: (p) => (p.mineRingEnabled = true),
    },
    {
        id: 'heal_pack_upgrade',
        name: 'Siphon Cells',
        desc: `Slaying monsters has a chance to drop a health pack that heals ${GAME_CONFIG.UPGRADES.SIPHON_CELLS_HEAL_HP} HP.`,
        oneShot: true,
        effect: (p) => {
            p.healPackEnabled = true;
            GAME_STATE.siphonCellsOwner = p;
        },
    },
    {
        id: 'melee_sledge_upgrade',
        name: 'Sledge Hammer',
        desc: `Melee attacks trigger a heavy forward cone slam in your movement direction, dealing ${GAME_CONFIG.UPGRADES.MELEE_SLEDGE_DAMAGE_PCT}% damage.`,
        oneShot: true,
        effect: (p) => (p.sledgeEnabled = true),
    },
    {
        id: 'melee_chain_upgrade',
        name: 'Scourge Flail',
        desc: 'Unlock a heavy melee spiked flail dragged behind you that swings with your movement.',
        oneShot: true,
        effect: (p) => p.unlockWeapon('player_flail'),
    },
    {
        id: 'flail_laser_upgrade',
        name: 'Laser Flail',
        desc: 'The Scourge Flail leaves a thin damaging laser trail along its motion path.',
        oneShot: true,
        effect: (p) => (p.flailLaserEnabled = true),
    },
    {
        id: 'melee_shield_upgrade',
        name: 'Iron Carapace',
        desc: `Reduce all incoming damage to you and your turrets by ${GAME_CONFIG.UPGRADES.MELEE_SHIELD_DAMAGE_REDUCTION_PCT}%.`,
        oneShot: true,
        effect: (p) =>
            (p.damageReduction =
                1 -
                GAME_CONFIG.UPGRADES.MELEE_SHIELD_DAMAGE_REDUCTION_PCT / 100),
    },
    {
        id: 'melee_reflect_upgrade',
        name: 'Barbed Carapace',
        desc: `Deals ${GAME_CONFIG.UPGRADES.REFLECT_DAMAGE_PLAYER_MAX_HP_PCT}% of your Max HP back as damage to any monster that damages you, and turrets reflect ${GAME_CONFIG.UPGRADES.REFLECT_DAMAGE_TURRET_MAX_HP_PCT}% max HP.`,
        oneShot: true,
        effect: (p) => (p.reflectDamageEnabled = true),
    },
    {
        id: 'melee_range_upgrade',
        name: 'Extended Joints',
        desc: `Increase the range of all melee attacks by ${GAME_CONFIG.UPGRADES.MELEE_RANGE_BOOST_PCT}%.`,
        oneShot: true,
        effect: (p) =>
            (p.meleeRangeModifier =
                1 + GAME_CONFIG.UPGRADES.MELEE_RANGE_BOOST_PCT / 100),
    },
    {
        id: 'rocket_upgrade',
        name: 'Seeking Rocket',
        desc: `${GAME_CONFIG.UPGRADES.ROCKET_PLAYER_CHANCE_PCT}% chance when firing magic missiles to launch a homing rocket that deals AoE damage.`,
        oneShot: true,
        effect: (p) => (p.rocketEnabled = true),
    },
    {
        id: 'buckshot_upgrade',
        name: 'Buckshot Volley',
        desc: `Magic missiles spawn ${GAME_CONFIG.UPGRADES.BUCKSHOT_SHRAPNEL_COUNT} smaller forward-spreading shrapnel projectiles upon hit, dealing 1/${GAME_CONFIG.UPGRADES.BUCKSHOT_SHRAPNEL_COUNT} damage.`,
        oneShot: true,
        effect: (p) => (p.buckshotEnabled = true),
    },
    {
        id: 'cluster_shot_upgrade',
        name: 'Cluster Shot',
        desc: 'Magic Missile and Buckshot shrapnel are replaced by explosive projectiles scaling with mine damage and AoE.',
        oneShot: true,
        effect: (p) => (p.clusterShotEnabled = true),
    },
    {
        id: 'projectile_lifedrain_upgrade',
        name: 'Warlock darts',
        desc: `Gain health equal to ${GAME_CONFIG.UPGRADES.PROJECTILE_LIFEDRAIN_PCT}% of the damage you deal with projectiles fired by you.`,
        oneShot: true,
        effect: (p) => (p.projectileLifedrainEnabled = true),
    },
    {
        id: 'campervan',
        name: 'Campervan Rampage',
        desc: `Transform into an invulnerable campervan for ${GAME_CONFIG.UPGRADES.CAMPERVAN_DURATION_SEC} seconds that crushes everything in its path.`,
        oneShot: true,
        effect: (p) => {
            p.campervanUntil =
    ...
    },
    {
        id: 'final_blast',
        name: 'Martyrdom',
        desc: 'Explode in a nuclear blast upon death, and permanently halve your respawn time.',
        oneShot: true,
        effect: (p) => {
            p.finalBlastEnabled = true;
            p.reviveTimeModifier = 0.5;
        },
    },
    {
        id: 'projectile_shield',
        name: 'Deflector Orbiters',
        desc: `Unlock an orbiting barrier of ${GAME_CONFIG.UPGRADES.DEFLECTOR_ORBITERS_COUNT} shields that block enemy projectiles. Absorbing a projectile destroys the shield, respawning after ${GAME_CONFIG.UPGRADES.DEFLECTOR_ORBITERS_RESPAWN_SEC}s. Scales with Attack Speed.`,
        oneShot: true,
        effect: (p) => p.unlockWeapon('projectile_shield'),
    },
    {
        id: 'heal',
        name: 'Second Wind',
        desc: 'Fully restore HP and double your Max HP.',
        effect: (p) => {
    ...
    },
    {
        ...
        name: "Martyr's Aura",
        desc: 'Leaves a healing aura when you are dead, damaging and slowing enemies.',
        oneShot: true,
        effect: (p) => {
            p.martyrdomAuraEnabled = true;
        },
    },
    {
        id: 'martyrs_presence_upgrade',
        name: "Martyr's Presence",
        desc: `Enlarges Martyr's Aura by ${GAME_CONFIG.UPGRADES.MARTYRS_PRESENCE_RADIUS_BOOST_PCT}%, knocks back surviving enemies on nuclear death, and provokes nearby enemies.`,
        oneShot: true,
        effect: (p) => {
            p.martyrsPresenceEnabled = true;
        },
    },
    {
        id: 'sacrificial_aegis_upgrade',
        name: 'Sacrificial Aegis',
        desc: `Reduces damage taken by nearby allies by ${GAME_CONFIG.UPGRADES.SACRIFICIAL_AEGIS_ALLY_REDUCTION_PCT}% (you take the blocked damage instead), and further reduces respawn time by ${GAME_CONFIG.UPGRADES.SACRIFICIAL_AEGIS_REVIVE_REDUCTION_PCT}%.`,
        oneShot: true,
        effect: (p) => {
            p.sacrificialAegisEnabled = true;
            p.reviveTimeModifier *=
                1 -
                GAME_CONFIG.UPGRADES.SACRIFICIAL_AEGIS_REVIVE_REDUCTION_PCT /
                    100;
        },
    },
    {
        ...
        desc: `Increases size by ${GAME_CONFIG.UPGRADES.CARAPACE_HEALER_SIZE_BOOST_PCT}%, and heals all players ${GAME_CONFIG.UPGRADES.CARAPACE_HEALER_TEAM_HEAL_PCT}% of your Max HP when taking damage.`,
        oneShot: true,
        effect: (p) => {
            p.carapaceHealerEnabled = true;
            p.r *=
                1 + GAME_CONFIG.UPGRADES.CARAPACE_HEALER_SIZE_BOOST_PCT / 100;
        },
    },
    {
        id: 'ice_trail_upgrade',
        name: 'Glacial Slide',
        desc: `Increase movement speed by another ${GAME_CONFIG.UPGRADES.ICE_TRAIL_SPEED_BOOST_PCT}%, leave a slowing ice trail, and gain immunity to fire damage while on ice.`,
        oneShot: true,
        effect: (p) => {
            p.speed *= 1 + GAME_CONFIG.UPGRADES.ICE_TRAIL_SPEED_BOOST_PCT / 100;
            p.iceTrailEnabled = true;
        },
    },
];
...
function upgradeWeight(u, player) {
    if (u.id === 'heal') {
        ...
        return Math.max(0, 0.4 - 0.08 * takenCount + missingPctBelow100 * 0.01); // 0: at most 1.39; 5: at most 0.99; 10; at most 0.59
    }
    return 1;
}
...
function pickThreeFor(player) {
    if (
        player?.currentUpgradeOptions &&
        player.currentUpgradeOptions.length === 3
    ) {
    ...
    const pool = UPGRADE_POOL.filter((u) => {
        if (
            ...
            (u.id === 'dash' ||
                u.id === 'dash_lvl2' ||
                u.id === 'phase_detonation_upgrade')
        )
            return false;
        if (
            u.id === 'unlock_ring' &&
            player.weapons.some((w) => w.id === 'fire_ring')
        )
            return false;
        if (u.id === 'unlock_missile' && hasMissile) return false;
        if (
            u.id === 'unlock_melee' &&
            player.weapons.some((w) => w.id === 'melee_sweep')
        )
            return false;
        ...
        if (
            u.id === 'building_duration_upgrade' &&
            (!hasTurret || player.buildingDurationCount >= 4)
        )
            return false;
        if (
            u.id === 'turret_cooldown_upgrade' &&
            (!hasTurret || player.turretCooldownCount >= 4)
        )
            return false;
        if (u.id === 'turret_flamethrower_upgrade' && !hasTurret) return false;
        if (
            u.id === 'turret_inferno_ring_upgrade' &&
            (!taken.has('turret_flamethrower_upgrade') || !hasRing)
        )
            return false;
        if (
        ...
        if (
            ...
            return false;
        if (
            u.id === 'turret_saw_upgrade' &&
            (!player.weapons.some((w) => w.id === 'melee_sweep') || !hasTurret)
        )
            return false;
        if ((u.id === 'accuracy' || u.id === 'multishot') && !hasMissile)
            return false;
        if (
            u.id === 'instant_missile_upgrade' &&
            (!taken.has('accuracy') || !hasMissile)
        )
            return false;
        if (u.id === 'sniper_shot_upgrade' && !taken.has('accuracy'))
            return false;
        if (
            u.id === 'laser_sniper_upgrade' &&
            (!taken.has('sniper_shot_upgrade') || !taken.has('speed_lvl2'))
        )
            return false;
        if (
            u.id === 'freeze_upgrade' &&
            ...
            return false;
        if (
            u.id === 'cryo_mine_upgrade' &&
            (!player.freezeEnabled || player.mineAoeCount < 1)
        )
            return false;
        if (
            u.id === 'mine_aoe_upgrade' &&
            (!hasMine || player.mineAoeCount >= 3)
        )
            return false;
        if (
            u.id === 'mine_attract_upgrade' &&
            !taken.has('explosion_heal_upgrade')
        )
            return false;
        if (u.id === 'mine_scatter_upgrade' && !hasMine) return false;
        if (
            u.id === 'mine_launcher_upgrade' &&
        ...
        if (
            u.id === 'mine_ring' &&
            (!hasRing || !hasMine || !taken.has('mine_launcher_upgrade'))
        )
            return false;
        if (
            u.id === 'melee_sledge_upgrade' &&
            !player.weapons.some((w) => w.id === 'melee_sweep')
        )
            return false;
        if (
            u.id === 'melee_chain_upgrade' &&
            (!player.weapons.some((w) => w.id === 'melee_sweep') ||
                !taken.has('melee_sledge_upgrade'))
        )
            return false;
        if (
            u.id === 'flail_laser_upgrade' &&
            (!hasFlail || !taken.has('speed_lvl2'))
        )
            return false;
        if (
            u.id === 'melee_shield_upgrade' &&
            !player.weapons.some((w) => w.id === 'melee_sweep')
        )
            return false;
        if (
            u.id === 'melee_reflect_upgrade' &&
            !taken.has('melee_shield_upgrade')
        )
            return false;
        if (
            u.id === 'melee_range_upgrade' &&
            !player.weapons.some((w) => w.id === 'melee_sweep')
        )
            return false;
        if (u.id === 'rocket_upgrade' && (!hasMissile || !hasMine))
            return false;
        if (u.id === 'speed_lvl2' && !taken.has('speed')) return false;
        if (u.id === 'dash_lvl2' && !taken.has('dash')) return false;
        if (
            u.id === 'phase_detonation_upgrade' &&
            (!taken.has('dash_lvl2') || !hasMine)
        )
            return false;
        if (
            u.id === 'buckshot_upgrade' &&
            (!hasMissile || player.multiShot === 0)
        )
            return false;
        if (
            u.id === 'cluster_shot_upgrade' &&
            (!taken.has('buckshot_upgrade') ||
                !taken.has('mine_launcher_upgrade'))
        )
            return false;
        if (
            u.id === 'projectile_lifedrain_upgrade' &&
            (!taken.has('buckshot_upgrade') || !taken.has('accuracy'))
        )
            return false;
        if (
            u.id === 'heal_pack_upgrade' &&
            (GAME_STATE.elapsed < 180000 ||
                GAME_STATE.players.some((p) => p.healPackEnabled) ||
                (GAME_STATE.siphonCellsOwner !== null &&
                    GAME_STATE.siphonCellsOwner !== player))
        )
            return false;
        if (u.id === 'martyrdom_aura_upgrade' && !taken.has('final_blast'))
            return false;
        if (
            u.id === 'martyrs_presence_upgrade' &&
            !taken.has('martyrdom_aura_upgrade')
        )
            return false;
        if (
            u.id === 'sacrificial_aegis_upgrade' &&
            !taken.has('martyrs_presence_upgrade')
        )
            return false;
        if (
            u.id === 'carapace_healer_upgrade' &&
            !taken.has('melee_reflect_upgrade')
        )
            return false;
        if (
            u.id === 'ice_trail_upgrade' &&
            (!taken.has('speed_lvl2') || !taken.has('freeze_upgrade'))
        )
            return false;
        if (u.id === 'cooldown' && player.attackSpeedCount >= 7) return false;
        if (
            u.id === 'campervan' &&
            (player.hp > player.maxHp * 0.5 || GAME_STATE.enemies.length < 200)
        )
            return false;
        if (
            u.id === 'final_blast' &&
            (!hasMine ||
                GAME_STATE.players.length < 2 ||
                player.mineAoeCount < 1)
        )
            return false;
        if (u.id === 'projectile_shield' && GAME_STATE.elapsed < 420000)
            return false;
    ...
    while (out.length < 3) {
        const base = UPGRADE_POOL.filter((u) => {
            ...
            if (
                ...
                return false;
            if (u.id === 'cooldown' && player.attackSpeedCount >= 7)
                return false;
            return true;
        });
...
const UPGRADE_DETAILS = {
    speed: {
        ...
        caveats:
            'Stacks multiplicatively with difficulty modifiers and player count speed scaling.',
    },
    speed_lvl2: {
        ...
        caveats:
            'Laser trail damage density increases when moving fast and circling enemies.',
    },
    damage: {
        ...
        caveats:
            'Damage increases multiply with difficulty damage multipliers.',
    },
    cooldown: {
    ...
    accuracy: {
        ...
        caveats:
            'Transforms missile flight trajectory from curved spread arcs into straight beams.',
    },
    instant_missile_upgrade: {
        ...
        caveats:
            'Shield Bearer energy barriers can still block line-of-fire between the shooter and the monster.',
    },
    sniper_shot_upgrade: {
        stacking: 'One-shot permanent unlock.',
        ...
        caveats:
            'Prioritizes the highest-HP target rather than the closest target.',
    },
    ...
    multishot: {
        stacking:
            'Can be selected multiple times without limit. Each selection adds +1 targeted missile per volley.',
        how: 'Each stack allows Magic Missiles to target +1 additional distinct enemy (targeting the next closest enemies in range).',
        caveats:
            'If fewer enemies are present than total missile count, extra missiles fire at the same primary target.',
    },
    dash: {
        stacking: 'One-shot permanent unlock.',
    ...
    phase_detonation_upgrade: {
        ...
        caveats:
            'Explosion AoE and damage scale with your Volatile Powder and mine damage modifiers.',
    },
    unlock_ring: {
        stacking: 'One-shot permanent unlock.',
    ...
    unlock_mine: {
        ...
        caveats:
            'Mines have a 500ms arming delay before they become triggerable.',
    },
    unlock_turret: {
        stacking: 'One-shot permanent unlock.',
        ...
        caveats:
            'Turrets have their own HP and can be damaged or destroyed by enemy attacks.',
    },
    laser_walls_upgrade: {
        stacking: 'One-shot permanent unlock.',
    ...
    turret_flamethrower_upgrade: {
        ...
        caveats:
            'The flamethrower cone fires independently of missile tracking.',
    },
    ...
    turret_dispenser_upgrade: {
        stacking: 'One-shot permanent unlock.',
        how:
            `Each active turret has a ${GAME_CONFIG.TURRET.DISPENSER_CHANCE_PCT}% chance every ${GAME_CONFIG.TURRET.DISPENSER_INTERVAL_SEC} seconds (scaled by Rapid Deployment) to dispense one of 8 distinct supply drops with equal probability:<br><br>` +
        ...
        caveats:
            'More active turrets on the field provide more dispensing opportunities.',
    },
    mine_aoe_upgrade: {
    ...
    mine_attract_upgrade: {
        ...
        caveats:
            'Requires Blast Mending. Attracted enemies are pulled towards the center of the mine.',
    },
    freeze_upgrade: {
        stacking: 'One-shot permanent unlock.',
        ...
        caveats:
            'Bosses are immune to freeze, but all standard and elite enemies are affected.',
    },
    cryo_mine_upgrade: {
        stacking: 'One-shot permanent unlock.',
    ...
    mine_scatter_upgrade: {
        ...
        caveats:
            'Scattered mines arm and detonate with identical stats to normal mines.',
    },
    ...
    explosion_heal_upgrade: {
        ...
        caveats:
            'Only heals players who are physically inside the explosion radius when it detonates.',
    },
    mine_ring: {
        stacking: 'One-shot permanent unlock.',
        ...
        caveats:
            'Each charge enters an individual regeneration cooldown after exploding.',
    },
    heal_pack_upgrade: {
        ...
        caveats:
            'Available starting at Minute 3; only one player in a co-op match can take this upgrade.',
    },
    ...
    melee_chain_upgrade: {
        ...
        caveats:
            'Circling enemies at high speed increases flail rotational velocity and damage.',
    },
    flail_laser_upgrade: {
        stacking: 'One-shot permanent unlock.',
    ...
    melee_range_upgrade: {
        ...
        caveats:
            'Affects Melee Sweep, Sledge Hammer, Scourge Flail, and Sawblade Turrets.',
    },
    rocket_upgrade: {
        stacking: 'One-shot permanent unlock.',
    ...
    cluster_shot_upgrade: {
        ...
        caveats:
            'Cluster munitions explode on impact and are blocked by Shield Bearer arcs.',
    },
    projectile_lifedrain_upgrade: {
        ...
        caveats:
            'Only applies to projectile damage originating from the player.',
    },
    campervan: {
        ...
        caveats:
            'Only offered in critical situations when HP is below 50% and 200+ enemies are on screen.',
    },
    final_blast: {
        stacking: 'One-shot permanent unlock.',
    ...
    heal: {
        ...
        caveats:
            'Doubling Max HP increases Barbed Carapace reflect damage and Sympathetic Shell healing.',
    },
    martyrdom_aura_upgrade: {
        stacking: 'One-shot permanent unlock.',
    ...
    carapace_healer_upgrade: {
        ...
        caveats:
            'Larger hitbox makes dodging harder, but triggers frequent team healing upon taking damage.',
    },
    ice_trail_upgrade: {
        stacking: 'One-shot permanent unlock.',
        ...
        caveats:
            'Ice trail persists for several seconds behind your movement path.',
    },
};
        ...
        for (const [childId, parents] of Object.entries(UPGRADE_DEPENDENCIES)) {
            if (parents?.includes(upgradeId)) {
                ...
                unlocks.push({
                    ...
                    name: childName,
                    desc: childDesc,
                    otherParents: uniqueOtherParents,
                });
            }
        }
    }
...
function showUpgradeDetailModal(u) {
    ...
    // 3. Caveats
    const caveatsSection = document.getElementById(
        'upgradeDetailCaveatsSection',
    );
    ...
    if (!unlocks || unlocks.length === 0) {
        unlocksList.innerHTML =
            '<span style="color:#888;">No further downstream upgrades depend on this upgrade.</span>';
    } else {
        unlocksList.innerHTML =
            ...
            unlocks
                .map((item) => {
                    const reqStr =
                        item.otherParents && item.otherParents.length > 0
                            ? ` <span style="color:#aaa; font-size:12px;">(also requires: <i>${item.otherParents.join(', ')}</i>)</span>`
                            : '';
                    const descStr = item.desc
                        ? `<div style="font-size: 12px; color: #bbb; margin-top: 2px; margin-bottom: 6px; line-height: 1.35;">${item.desc}</div>`
                        : '';
...
function initUpgradeDetailModalBindings() {
    const closeUpgradeDetailBtn = document.getElementById(
        'closeUpgradeDetailBtn',
    );
    if (closeUpgradeDetailBtn)
        closeUpgradeDetailBtn.onclick = closeUpgradeDetailModal;

    const closeUpgradeDetailBtnBottom = document.getElementById(
        'closeUpgradeDetailBtnBottom',
    );
    if (closeUpgradeDetailBtnBottom)
        closeUpgradeDetailBtnBottom.onclick = closeUpgradeDetailModal;
