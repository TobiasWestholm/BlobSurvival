...
const SoundEngine = {
    ...
    getSpeedProgress() {
        const p = Math.max(
            0.0,
            Math.min(1.0, (this.musicElapsedMs || 0) / this.SPEED_MAX_MS),
        );
    ...
    getFilterProgress() {
        return Math.max(
            0.0,
            Math.min(1.0, (this.musicElapsedMs || 0) / this.FILTER_DURATION_MS),
        );
    },
    getFilterCutoff() {
    ...
    getReverbProgress() {
        return Math.max(
            0.0,
            Math.min(1.0, (this.musicElapsedMs || 0) / this.REVERB_DURATION_MS),
        );
    },
    ...
    // biome-ignore format: 4-beat musical measure layout
            ...
            if (!(++crush % ((bitCrush * 100) | 0))) {
                ...
                s =
                    (repeatTime
                        ? 1 -
                          tremolo +
                          tremolo * Math.sin((PI2 * i) / repeatTime)
                        : 1) *
    ...
    buildSfxrBuffer(ps) {
        ...
        if (ps.p_arp_mod >= 0)
            arpeggioMultiplier = 1 - Math.pow(ps.p_arp_mod, 2) * 0.9;
        else arpeggioMultiplier = 1 + Math.pow(ps.p_arp_mod, 2) * 10;
        let arpeggioTime = Math.floor(
            Math.pow(1 - ps.p_arp_speed, 2) * 20000 + 32,
        );
        if (ps.p_arp_speed === 1) arpeggioTime = 0;

        const waveShape = parseInt(ps.wave_type);
        let fltw = Math.pow(ps.p_lpf_freq, 3) * 0.1;
        const enableLowPassFilter = ps.p_lpf_freq !== 1;
        const fltw_d = 1 + ps.p_lpf_ramp * 0.0001;
        let fltdmp =
            (5 / (1 + Math.pow(ps.p_lpf_resonance, 2) * 20)) * (0.01 + fltw);
        if (fltdmp > 0.8) fltdmp = 0.8;
        let flthp = Math.pow(ps.p_hpf_freq, 2) * 0.1;
        const flthp_d = 1 + ps.p_hpf_ramp * 0.0003;

        const vibratoSpeed = Math.pow(ps.p_vib_speed, 2) * 0.01;
        const vibratoAmplitude = ps.p_vib_strength * 0.5;
        ...
        let sample_sum = 0,
            num_summed = 0,
            summands = Math.floor(44100 / sampleRate) || 1;
        ...
        for (let t = 0; ; ++t) {
            ...
            if (vibratoAmplitude > 0) {
                ...
                rfperiod =
                    period * (1 + Math.sin(vibratoPhase) * vibratoAmplitude);
            }
            ...
            for (let si = 0; si < OVERSAMPLING; ++si) {
                ...
                if (waveShape === SQUARE) {
                    ...
                    sub_sample =
                        noise_buffer[Math.floor((phase * 32) / iperiod)];
                }
    ...
    init() {
        ...
        try {
            ...
            this.musicMidEq.frequency.setValueAtTime(
                1000,
                this.ctx.currentTime,
            );
            ...
            this.musicHighEq.frequency.setValueAtTime(
                5000,
                this.ctx.currentTime,
            );
            ...
            this.uiBus.gain.setValueAtTime(
                this.isMuted ? 0 : 0.8,
                this.ctx.currentTime,
            );
            ...
            this.fxReverbConvolverStart.buffer = this.buildImpulseResponse(
                4.5,
                Math.pow(2.2, 3),
            ); // 4.5s, cubic decay ~10.65

            this.fxReverbConvolverEnd = this.ctx.createConvolver();
            this.fxReverbConvolverEnd.buffer = this.buildImpulseResponse(
                1.6,
                Math.pow(1.7, 3),
            ); // 1.6s, cubic decay ~4.91

            this.fxReverbConvolverVictory = this.ctx.createConvolver();
            this.fxReverbConvolverVictory.buffer = this.buildImpulseResponse(
                9.0,
                Math.pow(1.8, 3),
            ); // 9.0s, cubic decay ~5.83

            this.fxWetGainStart = this.ctx.createGain();
            this.fxWetGainStart.gain.setValueAtTime(0.4, this.ctx.currentTime); // Starts at 20% wet return
            ...
            this.fxWetGainVictory.gain.setValueAtTime(
                0.0,
                this.ctx.currentTime,
            );
            ...
            this.musicReverbConvolverStart.buffer = this.buildImpulseResponse(
                4.5,
                2.2,
            );

            this.musicReverbConvolverEnd = this.ctx.createConvolver();
            this.musicReverbConvolverEnd.buffer = this.buildImpulseResponse(
                1.6,
                1.7,
            );

            this.musicReverbConvolverVictory = this.ctx.createConvolver();
            this.musicReverbConvolverVictory.buffer = this.buildImpulseResponse(
                9.0,
                1.8,
            );

            this.musicWetGainStart = this.ctx.createGain();
            this.musicWetGainStart.gain.setValueAtTime(
                0.68,
                this.ctx.currentTime,
            );
            ...
            this.musicWetGainVictory.gain.setValueAtTime(
                0.0,
                this.ctx.currentTime,
            );
            ...
            this.dashBuffer = this.buildZzfxBuffer([
                0.4,
                ,
                391.9954,
                0.06,
            ...
            this.meleeBuffer = this.buildZzfxBuffer([
                0.25,
                ,
                2600,
                0.015,
            ...
            this.flameBuffer = this.buildZzfxBuffer([
                0.4,
                ,
                84,
                0.37,
            ...
            this.freezeBuffer = this.buildZzfxBuffer([
                ,
                ,
                1e4,
                ,
            ...
            const meteorFallScale =
                METEOR_FALL_MS / 1000 / (0.58 + 0.59 + 0.01 + 0.008 + 0.1);
            this.meteorFallBuffer = this.buildZzfxBuffer([
                0.3,
                undefined,
                200,
                0.58 * meteorFallScale,
                0.01 * meteorFallScale,
                0.008 * meteorFallScale,
                2,
                0.3,
                undefined,
                -0.1,
            ...
            this.tentacleLashBuffer = this.buildZzfxBuffer([
                1.3,
                ,
                120,
                0.02,
            ...
            this.stalkerBlinkBuffer = this.buildZzfxBuffer([
                0.75,
                ,
                520,
                0.08,
            ...
            this.felhoundGallopBuffer = this.buildZzfxBuffer([
                ,
                ,
                48,
                0.019,
            ...
            this.medivacHealBuffer = this.buildZzfxBuffer([
                1.1,
                0,
                247,
                0.22,
            ...
            this.hellionFlameBuffer = this.buildZzfxBuffer([
                0.75,
                ,
                95,
                0.08,
            ...
            this.warpAnomalyBuffer = this.buildZzfxBuffer([
                1.5,
                ,
                40,
                0.02,
            ...
            this.viperTongueBuffer = this.buildZzfxBuffer([
                1.1,
                ,
                332,
                0.03,
            ...
            this.titanSprintBuffer = this.buildZzfxBuffer([
                ,
                ,
                110,
                0.2,
            ...
            this.titanUndergroundBuffer = this.buildZzfxBuffer([
                ,
                ,
                200,
                0.12,
            ...
            this.behemothCleaveBuffer = this.buildZzfxBuffer([
                0.8,
                ,
                40,
                0.03,
            ...
            this.damageBuffer = this.buildZzfxBuffer([
                2.3,
                ,
                300,
                0.04,
            ...
            this.bossWarningBuffer = this.buildZzfxBuffer([
                1.2,
                ,
                120,
                0.2,
            ...
            try {
                ...
                if (
                    localStorage.getItem('blob_survival_music_muted') === 'true'
                ) {
    ...
    setMuffled(muffled, durationSeconds = 0.5) {
        ...
        const targetFreq = muffled
            ? Math.min(300, progressCutoff)
            : progressCutoff;
    ...
    startMenuMusic(forceReset = false) {
        ...
        if (this.ctx) {
            ...
            if (this.musicFilter) {
                this.musicFilter.frequency.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.musicFilter.frequency.setValueAtTime(
                    400,
            ...
            if (this.musicWetGainStart) {
                this.musicWetGainStart.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.musicWetGainStart.gain.setValueAtTime(
                    0.68,
            ...
            if (this.musicWetGainEnd) {
                this.musicWetGainEnd.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.musicWetGainEnd.gain.setValueAtTime(
                    0.0,
                    this.ctx.currentTime,
                );
            }
            if (this.musicWetGainVictory) {
                this.musicWetGainVictory.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.musicWetGainVictory.gain.setValueAtTime(
                    0.0,
            ...
            if (this.fxWetGainStart) {
                this.fxWetGainStart.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.fxWetGainStart.gain.setValueAtTime(
                    0.2,
                    this.ctx.currentTime,
                );
            }
            if (this.fxWetGainEnd) {
                this.fxWetGainEnd.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.fxWetGainEnd.gain.setValueAtTime(
                    0.0,
                    this.ctx.currentTime,
                );
            }
            if (this.fxWetGainVictory) {
                this.fxWetGainVictory.gain.cancelScheduledValues(
                    this.ctx.currentTime,
                );
                this.fxWetGainVictory.gain.setValueAtTime(
                    0.0,
    ...
    triggerVictoryRamp(durationSeconds = 3.0) {
        ...
        this.victoryStartBpm =
            (10 + (140 - 10) * currentSpeedProg) * (this.tempoFactor || 1.0);
        this.victoryStartWet =
            this.musicWetGainEnd &&
            Number.isFinite(this.musicWetGainEnd.gain.value)
                ? this.musicWetGainEnd.gain.value
                : 0.68;
        this.victoryStartFxWet =
            this.fxWetGainEnd && Number.isFinite(this.fxWetGainEnd.gain.value)
                ? this.fxWetGainEnd.gain.value
                : 0.2;
    },
    ...
    updateMusic(now) {
        ...
        if (this.musicMode === 'menu') {
            ...
            if (
                this.musicWetGainStart &&
                this.musicWetGainEnd &&
                this.ctx &&
                !this.isMuffled
            ) {
                this.musicWetGainStart.gain.setTargetAtTime(
                    0.68,
                    this.ctx.currentTime,
                    0.05,
                );
                this.musicWetGainEnd.gain.setTargetAtTime(
                    0.0,
                    this.ctx.currentTime,
                    0.05,
                );
        ...
        if (this.isVictoryRamping) {
            ...
            const t = Math.min(
                1.0,
                Math.max(0.0, elapsed / this.victoryDuration),
            );
            ...
            if (this.musicWetGainEnd && this.musicWetGainVictory && this.ctx) {
                ...
                this.musicWetGainVictory.gain.setTargetAtTime(
                    victoryGain,
                    this.ctx.currentTime,
                    0.05,
                );
            ...
            if (this.fxWetGainEnd && this.fxWetGainVictory && this.ctx) {
                const endFxGain = (this.victoryStartFxWet || 0.2) * (1.0 - t);
                const victoryFxGain = 0.25 * t;
                this.fxWetGainEnd.gain.setTargetAtTime(
                    endFxGain,
                    this.ctx.currentTime,
                    0.05,
                );
                this.fxWetGainVictory.gain.setTargetAtTime(
                    victoryFxGain,
                    this.ctx.currentTime,
                    0.05,
                );
                if (this.fxWetGainStart) {
                    this.fxWetGainStart.gain.setTargetAtTime(
                        0.0,
                        this.ctx.currentTime,
                        0.05,
                    );
            ...
            while (this.musicNextNoteTime < this.ctx.currentTime + lookahead) {
                ...
                const noteName =
                    this.MELODY_SEQUENCE[
                        this.musicStep % this.MELODY_SEQUENCE.length
                    ];
                if (noteName && noteName !== '-') {
        ...
        if (!this.isMuffled && GAME_STATE.current === STATES.GAMEPLAY) {
            if (
                typeof GAME_STATE.elapsed === 'number' &&
        ...
        if (this.tempoTransitionDuration > 0) {
            const elapsed =
                (now - this.tempoTransitionStartTime) /
                (this.tempoTransitionDuration * 1000);
            const t = Math.min(1.0, Math.max(0.0, elapsed));
            this.tempoFactor =
                this.tempoTransitionStartFactor +
                (this.targetTempoFactor - this.tempoTransitionStartFactor) * t;
        } else {
        ...
        if (
            ...
            if (
                Number.isFinite(progressCutoff) &&
                Number.isFinite(this.ctx.currentTime)
            ) {
                this.musicFilter.frequency.setTargetAtTime(
                    progressCutoff,
                    this.ctx.currentTime,
        ...
        if (
            ...
            const fxStartWet = 0.2 * Math.pow(invRevProg, 1.2);
            const fxEndWet = 0.2 * Math.pow(revProg, 0.8);
            if (
                Number.isFinite(fxStartWet) &&
                Number.isFinite(this.ctx.currentTime)
            ) {
                this.fxWetGainStart.gain.setTargetAtTime(
                    fxStartWet,
    ...
    allocateVoice(priority, soundId, durationSeconds, isUI = false) {
        ...
        if (this.activeVoices.length >= this.MAX_VOICES) {
            ...
            if (minIndex !== -1 && priority > minPriority) {
                ...
                try {
                    ...
                    stolen.gainNode.gain.setValueAtTime(
                        stolen.gainNode.gain.value,
                        now,
                    );
                    stolen.gainNode.gain.linearRampToValueAtTime(
                        0.0001,
                        now + 0.005,
                    );
        ...
        if (priority === this.PRIORITY.CRITICAL && this.duckingGain) {
            try {
                ...
                this.duckingGain.gain.setValueAtTime(
                    this.duckingGain.gain.value,
                    now,
                );
                ...
                this.duckingGain.gain.linearRampToValueAtTime(1.0, now + 0.5);
            } catch {}
        }
    ...
    bossWarning() {
        ...
        const duration = this.bossWarningBuffer?.duration
            ? this.bossWarningBuffer.duration
            : 1.27;
        const v = this.allocateVoice(
            this.PRIORITY.CRITICAL,
            'boss_warning',
    ...
    phaseDash() {
        ...
        const duration = this.dashBuffer?.duration
            ? this.dashBuffer.duration
            : 0.2;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'phase_dash',
    ...
    meleeSweep(isSledge = false) {
        ...
        const duration = isSledge
            ? 0.14
            : this.meleeBuffer?.duration
              ? this.meleeBuffer.duration
              : 0.06;
    ...
    flamethrower() {
        ...
        const duration = this.flameBuffer?.duration
            ? this.flameBuffer.duration
            : 1.05;
    ...
    missileFire(soundVolumeFactor = 1.0, isTurret = false) {
        ...
        if (this.missileBuffer) {
            ...
            voiceGain.gain.setValueAtTime(
                this.FX_VOICE_GAIN * 0.9 * soundVolumeFactor,
                now,
            );
            voiceGain.gain.exponentialRampToValueAtTime(0.001, now + duration);
    ...
    laserSniper() {
        ...
        const duration = this.laserBuffer?.duration
            ? this.laserBuffer.duration
            : 0.18;
        const v = this.allocateVoice(
            this.PRIORITY.MEDIUM,
            'laser_sniper',
    ...
    autonomousNetwork() {
        ...
        toneGain.gain.linearRampToValueAtTime(0.7, now + 0.2);
    ...
    enemyFreeze() {
        ...
        const duration = this.freezeBuffer?.duration
            ? this.freezeBuffer.duration
            : 0.05;
        const v = this.allocateVoice(
            this.PRIORITY.LOW,
            'enemy_freeze',
        ...
        if (this.freezeBuffer) {
            ...
            voiceGain.gain.setValueAtTime(this.FX_VOICE_GAIN * 0.1, now);
    ...
    meteorFall(durationSec = 1.2) {
        ...
        let buffer =
            Math.abs(dur - METEOR_FALL_MS / 1000) < 0.05
                ? this.meteorFallBuffer
                : null;
        if (!buffer) {
    ...
    dasherJump() {
        ...
        const duration = this.dasherJumpBuffer?.duration
            ? this.dasherJumpBuffer.duration
            : 0.18;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'dasher_jump',
    ...
    shooterFire() {
        ...
        const duration = this.shooterMissileBuffer?.duration
            ? this.shooterMissileBuffer.duration
            : 0.45;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'shooter_fire',
    ...
    tentacleLash() {
        ...
        const duration = this.tentacleLashBuffer?.duration
            ? this.tentacleLashBuffer.duration
            : 0.55;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'tentacle_lash',
    ...
    stalkerBlink() {
        ...
        const duration = this.stalkerBlinkBuffer?.duration
            ? this.stalkerBlinkBuffer.duration
            : 0.12;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'stalker_blink',
    ...
    felhoundGallop() {
        ...
        const duration = this.felhoundGallopBuffer?.duration
            ? this.felhoundGallopBuffer.duration
            : 0.07;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'felhound_gallop',
    ...
    hellionFlame() {
        ...
        const duration = this.hellionFlameBuffer?.duration
            ? this.hellionFlameBuffer.duration
            : 0.34;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'hellion_flame',
        ...
        if (this.hellionFlameBuffer) {
            ...
            voiceGain.gain.setValueAtTime(this.FX_VOICE_GAIN * 0.3, now);
    ...
    warpAnomaly() {
        ...
        const duration = this.warpAnomalyBuffer?.duration
            ? this.warpAnomalyBuffer.duration
            : 0.72;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'warp_anomaly',
    ...
    viperTongue() {
        ...
        const duration = this.viperTongueBuffer?.duration
            ? this.viperTongueBuffer.duration
            : 0.67;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'viper_tongue',
    ...
    titanSprint() {
        ...
        const duration = this.titanSprintBuffer?.duration
            ? this.titanSprintBuffer.duration
            : 1.0;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'titan_sprint',
    ...
    titanUnderground() {
        ...
        const duration = this.titanUndergroundBuffer?.duration
            ? this.titanUndergroundBuffer.duration
            : 1.0;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'titan_underground',
    ...
    behemothCleave() {
        ...
        const duration = this.behemothCleaveBuffer?.duration
            ? this.behemothCleaveBuffer.duration
            : 0.33;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'behemoth_cleave',
    ...
    medivacHeal() {
        ...
        const duration = this.medivacHealBuffer?.duration
            ? this.medivacHealBuffer.duration
            : 0.75;
        const v = this.allocateVoice(
            this.PRIORITY.HIGH,
            'medivac_heal',
