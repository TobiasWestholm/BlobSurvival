...
function startGame(playerCount, difficultyKey) {
    ...
    GAME_STATE.elapsed = GAME_STATE.testingMode
        ? GAME_STATE.testStartMinute * 60000
        : 0;
    ...
    if (GAME_STATE.testingMode) {
        ...
        if (
            testMin > 8 ||
            (testMin >= 8 && testMin !== 7.9 && testMs > 480000)
        ) {
            GAME_STATE.completedBosses.add('octopus');
            for (let i = 0; i < 4; i++)
                GAME_STATE.bossWarningsFired.add('octopus_warn_' + i);
        }
        if (testMin >= 12 || testMs >= 719000) {
            GAME_STATE.completedBosses.add('horde');
            for (let i = 0; i < 4; i++)
                GAME_STATE.bossWarningsFired.add('horde_warn_' + i);
        ...
        if (testMin > 16 || testMs > 960000) {
            ...
            for (let i = 0; i < 4; i++)
                GAME_STATE.bossWarningsFired.add('felhound_warn_' + i);
        }
        if (
            testMin > 24 ||
            (testMin >= 24 && testMin !== 23.9 && testMs > 1440000)
        ) {
            GAME_STATE.completedBosses.add('behemoth');
            for (let i = 0; i < 4; i++)
                GAME_STATE.bossWarningsFired.add('behemoth_warn_' + i);
        }
    }
    let startLevel = 1;
    let nextXp = LVL2_XP;
...
function update(dt, dtFactor, now) {
    ...
    for (const p of GAME_STATE.players) {
        ...
        if (!p.alive) {
            ...
            if (
                diedBeforeBoss &&
                now - p.deadAt >= REVIVE_MS * p.reviveTimeModifier
            ) {
                p.revive();
            }
    ...
    const isBehemothPhase =
        GAME_STATE.elapsed >= 1440000 ||
        GAME_STATE.activeBoss === 'behemoth' ||
        GAME_STATE.completedBosses.has('behemoth');
    const canSpawnNormally =
        !GAME_STATE.activeBoss &&
        !isBehemothPhase &&
        now >= GAME_STATE.resumeNormalSpawnAt;
    ...
    if (now - GAME_STATE.lastSpawn > 2000 || GAME_STATE.enemies.length === 0) {
        if (canSpawnNormally) {
            ...
            for (let i = 0; i < count && GAME_STATE.enemies.length < 500; i++)
                spawnEnemy(now);
        }
    }
...
function addXp(amount) {
    ...
    while (GAME_STATE.xp >= GAME_STATE.nextXp) {
        ...
        GAME_STATE.level++;
        GAME_STATE.nextXp =
            Math.floor(GAME_STATE.nextXp * XP_EXPONENTIAL) + XP_ADD_PER_LEVEL;
        gained++;
    }
    if (gained > 0) {
        SoundEngine.levelUp();
...
function isLastBossCleared(winCond) {
    if (winCond?.type !== 'boss') return false;
...
function loop(now) {
    ...
    if (now - GAME_STATE.fpsLastUpdate >= 250) {
        GAME_STATE.currentFps = Math.round(
            (GAME_STATE.fpsFrameCount * 1000) /
                (now - GAME_STATE.fpsLastUpdate),
        );
        GAME_STATE.fpsFrameCount = 0;
        GAME_STATE.fpsLastUpdate = now;
    }
        ...
        if (netManager.isClient) {
            ...
            for (const p of GAME_STATE.projectiles) {
                ...
                if (p.type === 'fire_ring' || p.type === 'deflector_shield') {
                    ...
                    if (owner?.alive) {
                        ...
                        // Phase-locked angular convergence to track host angle with 0 lag
                        if (p.targetAngle !== undefined) {
                            ...
                            p.angle =
                                ...
                                rotSpeed * dtFactor +
                                diff * 0.35 * dtFactor;
                            p.targetAngle += rotSpeed * dtFactor;
                        } else {
                            p.angle = (p.angle || 0) + rotSpeed * dtFactor;
            ...
            for (const rp of GAME_STATE.players) {
                ...
                if (rp.targetFacingAngle !== undefined) {
                    ...
                    while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
                    rp.facingAngle += angleDiff * 0.25 * dtFactor;
                }
            }
